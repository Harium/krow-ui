...
import com.harium.krow.ui.util.ReturnsPooledObject;
...
public class KrTextField extends KrWidget {
    ...
    public KrTextField() {
        ...
        setFocusable(true);
        setDefaultStyle(KrToolkit.getDefaultToolkit().getSkin().getStyle(KrTextField.class));
    ...
    protected void keyPressedEvent(KrKeyEvent event) {
        ...
        if (event.getKeycode() == Input.Keys.C && event.isCtrlDown() && textDocument.hasSelection()) {
            KrToolkit.getDefaultToolkit().writeToClipboard(textDocument.getSelectedText());
            event.accept();
            return;
        }

        if (event.getKeycode() == Input.Keys.V && event.isCtrlDown()) {
            textDocument.insertText(KrToolkit.getDefaultToolkit().readFromClipboard());
            event.accept();
            return;
        }
    ...
    protected void drawSelf(KrRenderer renderer) {
        ...
        Rectangle innerViewport = KrRectangles.rectangles(innerGeometry).shrink(getPadding()).value();
        boolean viewportClip = renderer.beginClip(innerViewport);

        String text = textDocument.getText();
        KrFontMetrics metrics = KrToolkit.getDefaultToolkit().fontMetrics();
    ...
    private void recalculateTextOffset() {
        ...
        Rectangle innerViewport = KrRectangles.rectangles(getGeometry(tmpRect)).shrink(getPadding()).value();

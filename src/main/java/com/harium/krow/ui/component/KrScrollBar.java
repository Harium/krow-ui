...
public class KrScrollBar extends KrWidget {
    ...
    public KrScrollBar(KrOrientation orientation) {
        this(orientation, KrToolkit.getDefaultToolkit().getSkin().getStyle(KrScrollBar.class));
    ...
    protected Rectangle getThumbGeometry(Rectangle thumbGeometry) {
        if (orientation == KrOrientation.VERTICAL) {
    ...
    protected float getTrackLength() {
        return orientation == KrOrientation.VERTICAL ? getHeight() : getWidth();
    }
    ...
    protected void mousePressedEvent(KrMouseEvent event) {
        ...
        if (!getThumbGeometry(tmpRect).contains(localMouseLocation)) {
            float positionOnTrack = orientation == KrOrientation.VERTICAL ? localMouseLocation.y : localMouseLocation.x;
    ...
    protected void mouseMoveEvent(KrMouseEvent event) {
        ...
        if (isDragging) {
            float delta = orientation == KrOrientation.VERTICAL ? event.getDeltaMove().y : event.getDeltaMove().x;
